...
- 把房间页最后的 socket 监听、按钮绑定和启动编排抽到 `static/js/room/room_bootstrap.js`，让 `_room_scripts.html` 更接近“状态 + 核心函数”。
- 把录屏和虚化重逻辑进一步抽到 `static/js/room/room_recording.js` 与 `static/js/room/room_virtual_background.js`，减少增强功能继续挤占房间主链路。
...
| P1 | `templates/partials/_room_scripts.html` | 仍承担房间启动、媒体按钮、录屏、背景虚化、聊天事件绑定、清理逻辑 | 逐步拆成 `room_bootstrap.js`、`room_media_controls.js`、`room_recording.js`、`room_virtual_background.js` |
...
- Move final room-page socket listeners and bootstrap orchestration into `static/js/room/room_bootstrap.js`.
- Move recording and virtual-background heavy logic into `static/js/room/room_recording.js` and `static/js/room/room_virtual_background.js`.
...
| P1 | `templates/partials/_room_scripts.html` | Still owns room bootstrap, media buttons, recording, background blur, chat binding, and cleanup | Gradually split into `room_bootstrap.js`, `room_media_controls.js`, `room_recording.js`, and `room_virtual_background.js` |
