...
| 层级 | 主要技术 / 文件 | 职责 |
| --- | --- | --- |
| 页面模板层 | `templates/pages/*.html` | 登录页、首页、会议页、历史页、账号页、管理员后台等页面骨架 |
| 前端交互层 | `static/js/room/*.js` | 处理按钮事件、调用接口、连接 Socket.IO 和 LiveKit、渲染聊天与视频界面 |
...
- `i18n/translations.py` 是主要翻译表，`zh` 和 `en` key 集当前保持一致。
- 页面模板优先用 `t('key')`；房间内部分运行时提示和浏览器能力提示使用显式中英文分支。
- `scripts/check_i18n.py` 用于检查模板中遗漏的硬编码中文，提交前应运行。
- 用户可以通过语言切换入口或 `/account` 默认语言偏好影响后续页面语言。

桌面端和手机端适配：

- 首页保留会议号、密码和复制邀请链接流程，已移除二维码扫描入口。
- 手机和平板不再被后端拦截，可进入登录、首页、会议和账号流程；摄像头、麦克风、屏幕共享能力仍取决于移动浏览器支持。
- 房间页保留桌面大屏、手机竖屏和手机横屏布局：电脑端保持会议网格和右侧聊天栏，竖屏优先视频和底部控制栏，横屏使用紧凑双栏缩小版。
- `static/js/room/room_livekit.js` 使用统一媒体发布参数，不再按 UA 或视口宽度分叉。
- `_room_scripts.html` 保留通用全屏、播放恢复和共享屏幕观看逻辑，不再维护小屏专用入口。
- `static/css/room.css` 现在是会议页自己的主布局层，负责顶部栏、控制区、视频区和聊天区；`static/css/style.css` 只保留共享页面壳层和非房间页通用规则。
...
### 5.1 老师问“视频底层怎么处理、有没有 YUV 源文件”时怎么答

可以直接回答：

> 这个项目不是自己写视频编解码器，也不是直接处理 `.yuv` 源文件。我在应用层调用浏览器 WebRTC 能力和 LiveKit SFU。浏览器负责采集摄像头/麦克风/屏幕，WebRTC 负责把媒体编码、加密并通过网络发送；LiveKit 作为 SFU 负责接收和转发已编码的媒体流。我的项目主要处理用户身份、房间权限、媒体 track 的开关、屏幕共享规则和界面状态。

更细一点可以分层讲：

| 层级 | 项目里谁负责 | 是否直接接触 YUV |
| --- | --- | --- |
| 摄像头采集 | 浏览器 `getUserMedia` / 系统媒体能力 | 项目拿到的是 `MediaStreamTrack`，不是 `.yuv` 文件 |
| 编码与网络传输 | 浏览器 WebRTC 栈 | WebRTC 内部可能用 YUV/I420/NV12 等像素格式，但项目代码不直接操作 |
| 多人转发 | LiveKit SFU | 主要转发已编码的 RTP/SRTP 媒体包，不是 Flask 在转发原始帧 |
| 业务控制 | Flask + Socket.IO | 只管登录、入会、权限、聊天、共享状态和 token |
| 本地增强 | MediaPipe + canvas 背景虚化 | 会处理浏览器内存中的视频帧，但仍不是读取 YUV 源文件 |
| 录屏导出 | `MediaRecorder` + 可选 `ffmpeg` | 产物是 WebM/MP4 容器；FFmpeg 可能内部解码/转封装，但项目不保存 YUV |

如果老师继续追问“底层到底是什么”，可以补充：

- 摄像头原始画面在驱动、操作系统和浏览器内部可能以 YUV、NV12、I420 或 RGBA 等格式存在。
- WebRTC 会根据浏览器和设备能力选择 VP8、H.264、VP9、AV1 等编码格式，再封装成 RTP/SRTP 包传输。
- LiveKit 的核心角色是 SFU，不是传统意义上把所有视频解码成 YUV 再重新合成一路视频的 MCU。它通常尽量转发编码后的流，并根据订阅、带宽和质量层做选择。
- 项目代码层面看不到 `.yuv` 文件，也没有手写像素格式转换或编码器；能看到的是 `MediaStreamTrack`、LiveKit track publication/subscription、HTML `<video>` 播放和少量 canvas/MediaPipe 增强。

一句话总结：

> 我这个项目“使用了 LiveKit/WebRTC 的视频处理和传输能力”，而不是“自己处理 YUV 源文件”。YUV 是更底层媒体栈可能使用的帧格式，项目没有把它作为文件或数据结构直接管理。
...
| 媒体能力 | 实现位置 |
| --- | --- |
| 摄像头 | `static/js/room/room_livekit.js` |
| 麦克风 | `static/js/room/room_livekit.js` |
| 屏幕共享视频 | `static/js/room/room_livekit.js` |
| 屏幕共享音频 | `static/js/room/room_livekit.js` |
...
| `i18n/translations.py` | 中英文界面文案 |
| `scripts/check_i18n.py` | 检查模板中未抽取的中文文案 |
...
| `templates/index.html` | 首页，负责创建会议和加入会议 |
| `templates/room.html` | 会议页入口 |
| `templates/partials/_room_layout.html` | 会议页布局骨架 |
| `templates/partials/_room_scripts.html` | 会议页主脚本入口，负责 Socket.IO 事件、按钮绑定和房间业务调度 |
...
| 文件 | 说明 |
| --- | --- |
| `static/js/room/room_livekit.js` | LiveKit 连接、摄像头、麦克风、屏幕共享、媒体轨道同步 |
| `static/js/room/room_chat.js` | 聊天消息渲染、@ 提及、表情面板、附件消息渲染 |
| `static/js/room/room_ui.js` | 会议界面渲染、布局调度、参与者卡片更新 |
| `static/js/room/room_utils.js` | 复制文本、HTML 转义、媒体 track 辅助函数等工具 |
| `static/js/room/room_diagnostics.js` | RTC / LiveKit 诊断信息展示 |
| `static/css/room.css` | 会议页响应式布局、视频区和聊天区主样式 |
| `static/css/style.css` | 通用页面样式 |
...
房间页核心脚本在 `templates/partials/_room_scripts.html`。页面加载后会先建立 Socket.IO 连接：
...
- 为 LiveKit 连接失败、屏幕共享限制、背景虚化模型加载失败和录屏转封装失败提供更细的前端提示。
...
## 4.1 i18n and Responsive Layout

i18n:

- `i18n/translations.py` is the main translation table, and the `zh` and `en` key sets currently match.
- Templates prefer `t('key')`; some in-room runtime and browser-capability prompts use explicit Chinese/English branches.
- `scripts/check_i18n.py` checks templates for missed hardcoded Chinese and should be run before submitting.
- Users can change the page language through the language switch or the default language preference in `/account`.

Desktop and mobile layout:

- The home page keeps room ID, password, and invite-link copy workflows. The QR scanner entry has been removed.
...
- The room page keeps desktop, phone portrait, and phone landscape layouts: desktop uses the meeting grid with a right chat column, portrait prioritizes video with a bottom control bar, and landscape uses a compact two-column layout.
- `static/js/room/room_livekit.js` uses unified media publishing settings instead of branching by UA or viewport width.
- `_room_scripts.html` keeps general fullscreen, playback recovery, and screen-share viewing logic without phone-specific entry points.
- `room.css` is now the dedicated meeting-page layout layer for the top bar, controls, video area, and chat area, while `style.css` keeps shared shell rules for non-room pages.
...
### 5.1 How to Answer "What Happens Under the Video Layer? Is There Any YUV Source File?"

A concise answer:

> This project does not implement a video codec and does not directly process `.yuv` source files. At the application layer, it uses browser WebRTC capabilities and LiveKit SFU. The browser captures camera, microphone, or screen media; WebRTC encodes, encrypts, and sends the media over the network; LiveKit receives and forwards encoded streams. My project mainly handles identity, room permission, media track toggles, screen-share rules, and UI state.

Layered explanation:

| Layer | Responsible component | Direct YUV handling? |
| --- | --- | --- |
| Camera capture | Browser `getUserMedia` / system media stack | The project receives `MediaStreamTrack`, not `.yuv` files |
| Encoding and network transport | Browser WebRTC stack | WebRTC may internally use YUV/I420/NV12, but project code does not manipulate it |
| Multi-party forwarding | LiveKit SFU | Mostly forwards encoded RTP/SRTP packets; Flask does not relay raw frames |
| Business control | Flask + Socket.IO | Handles login, joining, permission, chat, share state, and tokens |
| Local enhancement | MediaPipe + canvas virtual background | Processes in-memory browser frames, but still not project-managed YUV files |
| Recording export | `MediaRecorder` + optional `ffmpeg` | Outputs WebM/MP4; FFmpeg may decode/remux internally, but the project does not store YUV |

If asked what the lower layer really is:

- Camera frames may internally exist as YUV, NV12, I420, RGBA, or similar formats inside the driver, operating system, or browser.
- WebRTC chooses codecs such as VP8, H.264, VP9, or AV1 depending on browser and device support, then sends media as RTP/SRTP packets.
- LiveKit acts as an SFU, not as an MCU that always decodes every participant into YUV and composites one mixed video. It generally forwards encoded streams and selects quality layers according to subscription and bandwidth.
- In the project code, the visible abstractions are `MediaStreamTrack`, LiveKit track publication/subscription, HTML `<video>` playback, and limited canvas/MediaPipe processing.

One-sentence summary:

> This project uses LiveKit/WebRTC for video processing and transport; it does not directly manage YUV source files. YUV is a lower-level frame format that may exist inside the media stack, not a file or data structure handled by this application.
...
| --- | --- |
| Camera | `static/js/room/room_livekit.js` |
| Microphone | `static/js/room/room_livekit.js` |
| Screen share video | `static/js/room/room_livekit.js` |
| Screen share audio | `static/js/room/room_livekit.js` |
...
| `i18n/translations.py` | Chinese/English UI text |
| `scripts/check_i18n.py` | Checks templates for untranslated Chinese text |
...
| `templates/partials/_room_layout.html` | Meeting page layout skeleton |
| `templates/partials/_room_scripts.html` | Main room script entry for Socket.IO events, button binding, and room orchestration |
...
| File | Description |
| --- | --- |
| `static/js/room/room_livekit.js` | LiveKit connection, camera, microphone, screen share, media track sync |
| `static/js/room/room_chat.js` | Chat rendering, @ mentions, emoji panel, attachment rendering |
| `static/js/room/room_ui.js` | Meeting UI rendering, layout scheduling, participant card updates |
| `static/js/room/room_utils.js` | Copying text, HTML escaping, media track helpers |
| `static/js/room/room_diagnostics.js` | RTC / LiveKit diagnostics display |
| `static/css/room.css` | Meeting page responsive layout and chat/video styling |
| `static/css/style.css` | Shared page styles |
...
All viewport sizes use the same capture and publishing parameters to avoid maintaining multiple media configurations.
...
`templates/partials/_room_scripts.html` creates the socket:
...
- Add clearer frontend messages for LiveKit connection failure, screen-sharing limits, background-blur model loading failures, and recording remux failures.
