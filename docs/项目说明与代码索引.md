...
实现上，房间页会把原始摄像头流送入 MediaPipe Selfie Segmentation，再把分割结果绘制到 canvas，最后用 canvas 输出的 track 替换 LiveKit 摄像头 track。为了降低启动失败概率，代码只会使用仍处于 `live` 状态的摄像头 track；失败时会回退到原始摄像头，并清理未成功的处理流。常见失败原因包括：摄像头刚被关闭或重开、浏览器还没有恢复本地预览、模型资源加载失败、设备性能不足。
...
- 虚拟背景依赖本地摄像头 live track、浏览器模型和 canvas 处理；屏幕共享和录屏也比较消耗资源，弱设备上可能影响体验。
...
- 为 LiveKit 连接失败、移动端屏幕共享限制、虚拟背景模型加载失败和录屏转封装失败提供更细的前端提示。
