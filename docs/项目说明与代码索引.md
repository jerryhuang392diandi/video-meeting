...
- 房间页保留桌面大屏、手机竖屏和手机横屏布局：电脑端保持会议网格和右侧聊天栏，手机端优先使用文档流和上下滚动，避免悬浮层阻塞会议控制。
- `static/js/room/room_livekit.js` 使用统一 LiveKit 主链路；手机端摄像头发布使用更稳的低分辨率/低码率档位，屏幕共享仍按浏览器能力增强处理。
...
- The room page keeps desktop, phone portrait, and phone landscape layouts: desktop uses the meeting grid with a right chat column, while phones prefer normal document flow and vertical scrolling to avoid overlays blocking meeting controls.
