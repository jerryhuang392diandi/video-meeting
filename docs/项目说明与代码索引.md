...
- 房间页保留桌面大屏和窄屏通用布局：电脑端保持会议网格和右侧聊天栏，窄屏时控制区下移到主内容下方。
...
Desktop and narrow-width layout:
...
- The room page keeps desktop and narrow-width general layouts: desktop uses the meeting grid with a right chat column, and narrow widths move controls below the main stage.
