...
- 房间页现在按桌面、平板、手机三段布局：电脑端保持会议网格和右侧聊天栏，平板会把控制区下移到主内容下方，手机端聊天区域重排为底部面板，并处理触摸滚动和输入框可见性。
- `static/js/room/room_livekit.js` 会按移动端/桌面端选择不同媒体发布参数，手机端更偏保守。
- `templates/partials/_room_scripts.html` 对移动端全屏、iOS 视频全屏、横屏锁定、触摸恢复播放、屏幕共享观看做了专门处理。
...
- The room page now has separate desktop, tablet, and phone layouts. Desktop keeps the meeting grid with a right chat column, tablets move the controls below the main stage, and phones turn chat into a bottom sheet while preserving touch scrolling and input visibility.
- `static/js/room/room_livekit.js` selects different media publishing settings for mobile and desktop, with more conservative settings on phones.
